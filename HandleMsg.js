...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        const botNumber = await aruga.getHostNumber() + '@kzxcode'
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', 'Nihh..', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomkpopx, '', 'Nihh...', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
	...
	case 'lirik':
		...
		case 'klasmen':
            ...
            let i = 1;
            urut.forEach((klsmn) => {
            textKlas += i+". @"+klsmn.id.replace('@kzxocde', '')+" ➤ Rp"+formatin(klsmn.denda)+"\n"
            i++
            });
	    ...
	    case 'add':
	        ...
	        if (args.length !== 1) return aruga.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    await aruga.addParticipant(from,`${args[0]}@kzxcode`)
                } catch {
        ...
        case 'promote':
            ...
            await aruga.sendTextWithMentions(from, `Request diterima, menambahkan @${mentionedJidList[0].replace('@kzxcode', '')} sebagai admin.`)
            break
        case 'demote':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            ...
            await aruga.sendTextWithMentions(from, `Request diterima, menghapus jabatan @${mentionedJidList[0].replace('@kzxcode', '')}.`)
            break
        case 'bye':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
        ...
        case 'everyone':
            ...
            for (let i = 0; i < groupMem.length; i++) {
                hehex += '╠➥'
                hehex += ` @${groupMem[i].id.replace(/@kzxcode/g, '')}\n`
            }
            hehex += '╚═〘 *KZXCODE BOT* 〙'
        ...
        case 'ban':
            ...
            if (args[0] == 'add') {
                banned.push(args[1]+'@kzxcode')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'Success banned target!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@kzxcode')
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *KZXCODE* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *KZXCODE* 〙\n\n${msg}`)
            }
