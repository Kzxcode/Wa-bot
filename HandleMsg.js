...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai sayang~, Im KZXCODE BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, '✅ Ini stciker kamu')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                aruga.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    aruga.reply(from, '✅ Ini stciker kamu')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, '⏳Sedang di proses... ', id)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Ini kak..', id)
                } catch {
                    aruga.reply('⚠ Proses Gagal, Silahkan isi dengan benar', id)
                }
            } else {
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', '✅ Done', id)
            .catch(() => {
                aruga.reply(from, '⚠ Ada yang Error!', id)
            })
            break
        ...
        //Media
        case 'instagram':
            if (args.length == 0) return aruga.reply(from, `ℹ Untuk mendownload gambar atau video dari instagram\nketik: ${prefix}instagram [link_ig]`, id)
            ...
            break
        case 'ytmp3':
            if (args.length == 0) return aruga.reply(from, `ℹ Untuk mendownload lagu dari youtube\nketik: ${prefix}ytmp3 [link_yt]`, id)
            ...
            break
        case 'ytmp4':
            if (args.length == 0) return aruga.reply(from, `ℹ Untuk mendownload video dari youtube\nketik: ${prefix}ytmp3 [link_yt]`)
	...
	case 'lirik':
        ...
        case 'kick':
            ...
            await aruga.sendTextWithMentions(from, `✅ Permintaan diterima, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            ...
            break
        case 'promote':
            if (!isGroupMsg) return aruga.reply(from, '⚠ Maaf, \nperintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, '❌ Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, '➕Gagal, \nsilahkan tambahkan bot sebagai admin grup!', id)
            ...
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, '❌ Maaf, \nformat pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
        ...
        case 'bye':
            ...
            break
        case 'del':
            if (!isGroupAdmins) return aruga.reply(from, '⚠ Gagal, \nperintah ini hanya dapat digunakan oleh admin grup!', id)
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await aruga.reply(from, "⚠ Berbicara yang sopan ya\nAnda telah mengatakan kata kasar\nSebanyak : "+formatin(denda.denda), id)
                        }
                    }
                } else {
