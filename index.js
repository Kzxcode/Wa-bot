...
const start = (aruga = new Client()) => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai Honey~, Im Kzxcode BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Halll, Selamat datang di group @${event.who.replace('@c.us', '')} \n\nSemoga harimu menyenangkan 😊`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Selamat tinggal @${event.who.replace('@c.us', '')}, Semoga harimu menyenangkan 😊`)
        }
    })
